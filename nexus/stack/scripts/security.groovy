...
def allRepoNames = [repoDockerHostedName,
                 ...
                 repoDockerProxyName,
                 repoDockerGroupName].collect { "docker-$it" } +
        [repoMvnHostedName,
         repoMvnReleasesName,
         repoMvnProxy1Name,
         repoMvnProxy2Name,
         repoMvnGroupName].collect { "maven2-$it" }

def snapshotRepoNames = [repoDockerHostedName].collect { "docker-$it" } +
        [repoMvnHostedName].collect { "maven2-$it" }

def releaseRepoNames = [repoDockerHostedName].collect { "docker-$it" } +
        [repoMvnHostedName].collect { "maven2-$it" }
...
/*Create a new role that allows a user same access as anonymous and adds healtchcheck access*/
    def devPrivileges = ['nx-healthcheck-read', 'nx-healthcheck-summary-read', 'nx-search-read'] +
                allRepoNames.collect{"nx-repository-view-$it-read"} +
                allRepoNames.collect{"nx-repository-view-$it-browse"}
    def anoRole = []
...
/* privileges with pattern * to allow any format, browse and read are already part of nx-anonymous*/
    def depPrivileges = snapshotRepoNames.collect{"nx-repository-view-$it-add"} +
            snapshotRepoNames.collect{"nx-repository-view-$it-edit"}
    def roles = ['developer']
/* add roles that uses the developer role as a basis and adds more privileges*/
    ...
    def relPrivileges = releaseRepoNames.collect{"nx-repository-view-$it-add"} +
            releaseRepoNames.collect{"nx-repository-view-$it-edit"}
